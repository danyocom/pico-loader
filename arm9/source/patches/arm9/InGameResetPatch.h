...
///
///        The keys are sampled in the SDK interrupt dispatcher, which is library code
///        shared by every retail title, and the reset enters the same reboot into Pico
///        Loader that \see OSResetSystemPatch uses. That reboot is installed here when the
